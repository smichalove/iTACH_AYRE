...
All actions and errors are logged to /home/steven/logfile.txt.
"""
...
# PORT 1 (Ayre V-1xe): Uses the original Toggle code
POWER_TOGGLE_PORT_1: str = "sendir,1:1,1,36000,1,1,32,32,64,64,64,32,32,32,32,32,32,32,32,32,32,64,32,32,64,32,32,2487"

# PORT 3 (Ayre AX-7): Uses DISCRETE codes (CODE SET 2) with REPEAT COUNT 3
# ON: The 38kHz 'Toggle' pattern
AX7_DISCRETE_ON: str = "sendir,1:3,1,38000,3,1,32,32,64,64,64,32,32,32,32,32,32,32,32,32,32,64,32,32,64,32,32,2487"

# OFF: The 36kHz 'Code Set 2' pattern
AX7_DISCRETE_OFF: str = "sendir,1:3,1,36000,3,1,32,32,32,32,32,32,64,32,32,32,32,32,32,64,32,32,32,32,64,32,32,64,32,2487"
...
def wake_online(mac: Optional[str] = None, broadcast: Optional[str] = None) -> None:
    """
    Convenience wrapper to wake a single device (PC) using Wake-on-LAN.

    If `mac` or `broadcast` are not provided, uses the configured
    `WOL_MAC_ADDRESS` and `WOL_BROADCAST_ADDRESS` constants.
    """
    mac = mac or WOL_MAC_ADDRESS
    broadcast = broadcast or WOL_BROADCAST_ADDRESS

    logging.info(f"wake_online: sending WOL to {mac} via {broadcast}")
    try:
...
def monitor_sensor_and_toggle_on_change() -> None:
    ...
    triggers the relay pulse and IR commands.
    """
    ...
    try:
        ...
        current_state: str = response.split(',')[-1].strip()
        ...
        if current_state != last_state:
            ...
            if last_state == '0' and current_state == '1':
                ...
                # 1. Send IR power toggle command to Port 1 (V-1xe)
                logging.info("Sending IR power TOGGLE command to Port 1 (V-1xe)...")
                send_command(s, POWER_TOGGLE_PORT_1, "POWER_TOGGLE_IR_PORT1")
                time.sleep(1)

                # 2. Send DISCRETE ON command to Port 3 (AX-7)
                logging.info("Sending Discrete POWER ON command to Port 3 (AX-7)...")
                send_command(s, AX7_DISCRETE_ON, "POWER_ON_IR_PORT3")
                time.sleep(1)
            ...
            # --- BRANCH 2: Transition from ON to OFF ---
                ...
                try:
                    ...
                    # 2. Send DISCRETE OFF command to Port 3 (AX-7)
                    logging.info("Sending Discrete POWER OFF command to Port 3 (AX-7)...")
                    send_command(s, AX7_DISCRETE_OFF, "POWER_OFF_IR_PORT3")
                    time.sleep(1) 
                ...
                # Ensure the amplifier relay is pulsed once regardless of IR command success.
                logging.info("Pulsing relay on IP2CC...")
                # pulse_ip2cc_relay()
