...
class TestPowerd(unittest.TestCase):
    ...
    @patch('time.sleep')
    def test_monitor_off_to_on(self, mock_sleep, mock_get_last_state, mock_create_conn, mock_send_cmd, mock_pulse, mock_set_state):
        """
        Test the OFF-to-ON transition.
        ...
        powerd.monitor_sensor_and_toggle_on_change()

        # Check that the relay was pulsed LAST
        mock_pulse.assert_called_once()

        # Verify that the 1-second delays are present (Grid protection)
        mock_sleep.assert_has_calls([call(1), call(1)])

        # IR commands should be sent
        ir_calls = [
            call(mock_sock, powerd.POWER_TOGGLE_COMMAND, "POWER_TOGGLE_IR_PORT1"),
            call(mock_sock, powerd.POWER_TOGGLE_COMMAND.replace(",1:1,", ",1:3,"), "POWER_TOGGLE_IR_PORT3")
        ]
        mock_send_cmd.assert_has_calls(ir_calls, any_order=False)

        # Verify Order: IR then Pulse
        manager = MagicMock()
        ...
        expected_call_order = [
            ...
            call.send_command(mock_sock, powerd.POWER_TOGGLE_COMMAND, "POWER_TOGGLE_IR_PORT1"),
            call.send_command(mock_sock, powerd.POWER_TOGGLE_COMMAND.replace(",1:1,", ",1:3,"), "POWER_TOGGLE_IR_PORT3"),
            call.pulse_relay()
        ]
        self.assertEqual(manager.mock_calls, expected_call_order)
    ...
    @patch('time.sleep')
    def test_monitor_on_to_off(self, mock_sleep, mock_get_last_state, mock_create_conn, mock_send_cmd, mock_pulse, mock_set_state):
        """
        ...
        # Verify that the 1-second delays are present
        mock_sleep.assert_has_calls([call(1), call(1)])
